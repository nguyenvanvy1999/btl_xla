...
export function getLabel(value, colorData) {
	return colorData[value] !== 0 || colorData[value] !== undefined ? colorData[value].label : NONE;
}
	...
	if (band === 'multiplier') {
		colorValue =
			typeData[index].value !== -1 ? (
				<>
					- 10 <sup>{Math.round(Math.log(typeData[index].value) / Math.log(10))}</sup>
				</>
			) : null;
	} else if (band === 'tolerance') {
		colorValue = UNICODE_PLUS_MINUS + typeData[index].value;
	} else {
		colorValue = typeData[index].value !== -1 ? `- ${typeData[index].value}` : null;
	}
	return colorValue;
}
